...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Visualize UPSET gatepin ASET results and map nets to driver gates."
    )
    parser.add_argument(
        "csv_file",
        nargs="?",
    ...
    parser.add_argument(
        ...
        type=int,
        default=25,
        help="Number of top ASET entries to plot.",
    )
    parser.add_argument(
        "--min-aset",
        type=float,
        default=0.0,
        help="Minimum ASET value to include.",
    )
    parser.add_argument(
        "--ascending",
        action="store_true",
        help="Sort ASET in ascending order. Default is descending.",
    )
    parser.add_argument(
        "--out-png",
        default=None,
        help="Output PNG plot path. Default: <csv directory>/aset_gatepins.png",
    )
    parser.add_argument(
        "--out-csv",
        default=None,
        help="Output mapped CSV path. Default: <csv directory>/aset_gatepins_mapped.csv",
    )
...
def parse_verilog_instances(verilog_path: Path):
    """
    Parses simple gate-level Verilog instances like:
    ...
    Returns:
      driver_by_net[net] = {"gate": ..., "cell": ..., "pin": ...}
      net_by_gatepin["U282/Y"] = "N2891"
    """
...
def plot_top_aset(rows, out_png: Path, top_n: int, ascending: bool):
    selected = rows[:top_n]
    ...
    plt.title(f"Top {len(plot_rows)} gatepins/nodes by ASET")
    plt.tight_layout()
...
def main():
    ...
    out_png = Path(args.out_png) if args.out_png else csv_path.parent / "aset_gatepins.png"
    out_csv = Path(args.out_csv) if args.out_csv else csv_path.parent / "aset_gatepins_mapped.csv"
    ...
    rows = [row for row in rows if row["aset"] > args.min_aset]
    rows.sort(key=lambda row: row["aset"], reverse=not args.ascending)

    write_mapped_csv(rows, out_csv)
    print(f"Wrote mapped CSV: {out_csv}")

    plot_top_aset(rows, out_png, args.top, args.ascending)
